...
require_pattern "$ton_fee_quote" 'TonAPIClientFactory\.reviewedSendNetwork\(for: endpointURL\) == identity\.network' "TON fee quote reviewed-origin and network binding"
...
require_pattern "$ton_pending_journal" 'maximumRecordBytes = 512 \* 1024' "TON bounded journal record"
...
require_pattern "$ton_send_service" 'var coordinationKey: String \{ network == \.testnet \? "testnet:" \+ sender : sender \}' "TON network-separated pending-intent key"
require_multiline_pattern "$ton_send_service" 'static let shared = TonPendingIntentCoordinator\([[:space:]]*journal: TonKeychainPendingIntentJournal\(\)[[:space:]]*\)' "TON production coordinator Keychain journal binding"
require_pattern "$ton_send_service" 'journal\.load\(senderRaw: identity\.coordinationKey\)' "TON journal-first network-scoped sender recovery"
require_multiline_pattern "$ton_send_service" 'try journal\.save\(pending\)[[:space:]]+statesBySender\[pending\.identity\.coordinationKey\] = \.pending' "TON durable reserve before in-memory exposure state"
...
require_multiline_pattern "$ton_send_service" 'try journal\.save\(retained\)[[:space:]]*statesBySender\[pending\.identity\.coordinationKey\] = \.pending\(retained\)' "TON retained pending durable-save-before-state ordering"
...
require_multiline_pattern "$ton_send_service" '(?:^|\n)[ \t]*#if !DEBUG[ \t]*\r?\n[ \t]*guard request\.legacyNativePrivateKey != nil else \{[[:space:]]+throw TonSendServiceError\.productionSendDisabled[[:space:]]+\}[[:space:]]+#endif' "TON Release service legacy-key-only guard"
require_multiline_pattern "$ton_send_service" '(?:^|\n)[ \t]*#if !DEBUG[ \t]*\r?\n[ \t]*guard let nativeAccount = legacyAccount, nativeAccount\.publicKey == request\.publicKey,[[:space:]]+try TonSwift\.Address\.parse\(nativeAccount\.address\)\.toRaw\(\) == TonSwift\.Address\.parse\(request\.senderAddress\)\.toRaw\(\)[[:space:]]+else \{ throw TonSendServiceError\.productionSendDisabled \}[[:space:]]+#endif' "TON Release service matching legacy-account guard"
require_multiline_pattern "$ton_send_service" 'guard let key = credentials\.legacyNativePrivateKey else \{ throw TonSendServiceError\.invalidAccount \}[[:space:]]+_ = try legacyAccount\.validatedPrivateKey\(key\)' "TON legacy private-key validation before signing"
require_pattern "$ton_send_service" 'TonAPIClientFactory\.reviewedSendNetwork\(for: url\) == \(intent\.tonConnect\?\.network \?\? \.mainnet\)' "TON signed remote-operation network binding"
...
require_exact_pattern_count "$ton_send_service" 'try requireTrustedSignedOperationEndpoint\(\)' 4 "TON every signed remote operation endpoint guard"
...
require_pattern "$chain_registry" '^[[:space:]]*static let canonicalAuthenticatedOrigin = URL\(string: "https://tonapi\.io"\)!$' "TON immutable mainnet origin"
require_pattern "$chain_registry" '^[[:space:]]*static let canonicalTestnetOrigin = URL\(string: "https://testnet\.tonapi\.io"\)!$' "TON immutable testnet origin"
require_pattern "$chain_registry" 'static let reviewedProductionSendOrigins = \[canonicalAuthenticatedOrigin, canonicalTestnetOrigin\]' "TON binary-owned production endpoint allowlist"
require_pattern "$chain_registry" 'if hasExactOrigin\(url, canonical: canonicalAuthenticatedOrigin\) \{ return \.mainnet \}' "TON reviewed mainnet origin mapping"
require_pattern "$chain_registry" 'if hasExactOrigin\(url, canonical: canonicalTestnetOrigin\) \{ return \.testnet \}' "TON reviewed testnet origin mapping"
...
require_multiline_pattern "$transfer_service" '(?:^|\n)[ \t]*#if !DEBUG[ \t]*\r?\n[ \t]*guard wallet\.legacyTonAccount != nil else \{ throw TransferServiceError\.tonProductionSendDisabled \}[[:space:]]+#endif' "TON Release integration legacy-account-only guard"
