...
PRIVATE_REPO_DIR="${PRIVATE_REPO_DIR:-../fearless-iOS-priv}"
...
is_allowed_overlay_path() {
  case "$1" in
    README.md|LICENSE|.gitignore|Jenkinsfile)
      return 0
      ;;
    .github/*|docs/release*|docs/rollback*|docs/private-overlay*|metadata/*|release/*|fastlane/*)
      return 0
      ;;
    fearless/CIKeys.stencil|fearless/fearless.entitlements|fearless/Configs/fearless.release.xcconfig)
      return 0
      ;;
    scripts/secrets/*|scripts/audit-private-overlay-boundary.sh)
      return 0
      ;;
    *)
      return 1
...
if [[ -s "$unexpected" ]]; then
  ...
  echo "[private-overlay-audit][error] private iOS repo is not overlay-only." >&2
...
info "Private iOS overlay boundary passed."
