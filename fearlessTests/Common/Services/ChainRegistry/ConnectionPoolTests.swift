...
class ConnectionPoolTests: XCTestCase {
    ...
    func testConnectionStatRetrieval() {
        do {
            // given
            ...
            let setupConnection: () -> MockConnection = {
                ...
                stub(mockConnection.stateReporting) { stub in
                    stub.state.get.thenReturn(.connected)
                }
            ...
            let connections: [JSONRPCEngine] = try chainModels.reduce(
                []
            ) { (allConnections, chain) in
                let connection = try connectionPool.setupConnection(for: chain)
                return allConnections + [connection]
            ...
            let actualChainIds = Set(connectionPool.connections.keys)
            let stateIds = try connectionPool.getConnectionStates().map { $0.chainId }

            XCTAssertEqual(Set(stateIds), actualChainIds)
            XCTAssertEqual(connections.count, stateIds.count)
        } catch {
            XCTFail("Did receive error \(error)")
        }
    }
