...
Use this when a released iOS build causes a production-impacting issue.

## Trigger

- Stop or pause phased release when crash, signing, migration, transfer, or
...
- If binary rollback is required, halt phased release in App Store Connect and
  prepare a hotfix branch from `master`.
