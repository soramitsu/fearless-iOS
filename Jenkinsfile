...
node('mac-fearless') {
  withEnv(envList) {
    ...
    // Sanitize global Git URL rewrites before checkout to avoid protocol errors
    // e.g., literal entries like url."${GH_PAT_READ:+https...}".insteadOf "https://github.com/"
    sh '''
set -euo pipefail
echo "==> Cleaning broken global Git URL rewrites (if any)"

# Remove rewrite keys that contain unexpanded ${...} patterns (grep -F to avoid quoting pitfalls)
...
if [ -f fearless.xcworkspace/contents.xcworkspacedata ]; then
  ...
  if ! xcodebuild \
    ...
    if command -v rg >/dev/null 2>&1; then
      ...
      (grep -nE "(^| )error:|Failed frontend command" build/pr.build.raw.log || true) >&2
    fi
    exit 65
  fi
