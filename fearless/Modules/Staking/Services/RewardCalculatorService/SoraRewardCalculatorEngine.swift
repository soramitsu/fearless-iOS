...
final class SoraRewardCalculatorEngine: RewardCalculatorEngineProtocol {
    ...
    func calculateEarnings(
        ...
        let filteredRewardPointsByEra = rewardPointsByEra.sorted(by: { $0.key < $1.key }).prefix(rewardPointsByEra.count - 1)
        let filteredValidatorRewardsByEra = validatorRewardsByEra.sorted(by: { $0.key < $1.key }).prefix(validatorRewardsByEra.count - 1)
        let avgIndividualRewardPoint = filteredRewardPointsByEra
            .compactMap { $0.value.individual }
            .reduce([], +)
            .filter { $0.accountId == validatorAccountId }
            .compactMap { $0.rewardPoint }
            .average()
        let eraRewardPointsTotal = filteredRewardPointsByEra.compactMap { $0.value.total }.average()
        ...
        let averageValidatorPayout = filteredValidatorRewardsByEra.compactMap { $0.value }.average()
