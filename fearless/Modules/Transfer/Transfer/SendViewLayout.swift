...
final class SendViewLayout: UIView {
    ...
    let selectNetworkView = UIFactory.default.createNetworkView(selectable: false)
    ...
    let commentTextField: CommonInputView = {
        ...
        inputView.backgroundView.strokeWidth = 1
    ...
    func bind(viewModel: RecipientViewModel?) {
        ...
        searchView.updateState(icon: viewModel?.icon, clearButtonIsHidden: !(viewModel?.canEditing == true))
        searchView.isValid = viewModel?.isValid
    }
...
private extension SendViewLayout {
    func setupLayout() {
        ...
        contentView.stackView.addArrangedSubview(commentTextField)
    ...
    func applyLocalization() {
        ...
        commentTextField.title = R.string.localizable.commonMessage(preferredLanguages: locale.rLanguages)
    }
}
